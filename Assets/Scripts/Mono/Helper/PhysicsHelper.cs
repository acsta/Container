...
namespace TaoTie
{
    public static class PhysicsHelper
    {
        ...
        public static readonly int hitscene = LayerMask.GetMask("HitScene");
        ...
        public static int RaycastNonAllocEntity(Vector3 origin, Vector3 direction, float maxDistance, EntityType[] filter,
            out long[] res)
        {
            res = entities;
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                var e = hitInfo.transform.GetComponentInParent<EntityComponent>();
                if (e == null) continue;
                for (int j = 0; j < filter.Length; j++)
                {
                    if (e.EntityType == filter[j])
                    {
                        ...
                        entities[index] = e.Id;
                        Colliders[count] = hitInfo.collider;
                        count++;
                        break;
                    }
                }
        ...
        public static int RaycastNonAllocHitInfo(Vector3 origin, Vector3 direction, float maxDistance, EntityType[] filter,
            CheckHitLayerType type, out HitInfo[] res)
        {
            res = hitInfos;
            if (filter == null) return 0;
            var len = Physics.RaycastNonAlloc(origin, direction, raycastHits, maxDistance, GetHitLayer(type), QueryTriggerInteraction.Collide);
            return FilterHitInfo(filter, len, origin);
        }
