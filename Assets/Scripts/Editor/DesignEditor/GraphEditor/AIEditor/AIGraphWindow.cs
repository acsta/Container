...
namespace TaoTie
{
    public class AIGraphWindow : GraphWindow<AIGraph>
    {
        ...
        [MenuItem("Tools/工具/策划/Graph编辑器/AI编辑器")]
        ...
        protected override void AddGraphMenuItems(GenericMenu menu)
        {
            ...
            else
            {
                ...
                menu.AddItem(new GUIContent("Create/AiCompareNode"), false,
                    () => { CreateNodeView(m_Graph.CreateNode<AICompareNode>(current.mousePosition, "Compare")); });
            }
        }
        ...
        protected override void AddPortMenuItems(GenericMenu menu, Port port, bool isLine = false)
        {
            ...
            if (port.IsOutput())
            {
                ...
                if (nodeOutput is AIRootNode || nodeOutput is AIConditionNode || nodeOutput is AICompareNode)
                {
                    ...
                    menu.AddItem(new GUIContent($"{port.portName}Connect/AiCompareNode"), false, () =>
                    {
                        var node = m_Graph.CreateNode<AICompareNode>(current.mousePosition, "Compare");
                        CreateNodeView(node);
                        ConnectPorts(port, node.GetFirstInputPort());
                    });
                }
        ...
        private DecisionNode Convert(NodeBase aiNode)
        {
            ...
            if (aiNode is AIActionNode anode)
            {
                ...
                res.Emoji = anode.Data.Emoji;
                res.Delay = anode.Data.Delay;
                return res;
            }
            if (aiNode is AICompareNode cpnode)
            {
                var res = new DecisionCompareNode();
                res.CompareMode = cpnode.CompareMode;
                res.LeftValue = cpnode.LeftValue;
                res.RightValue = cpnode.RightValue;
                for (int i = 0; i < cpnode.outputPorts.Count; i++)
                {
                    if (cpnode.outputPorts[i].portName == "True")
                    {
                        if(cpnode.outputPorts[i].edges!=null && cpnode.outputPorts[i].edges.Count>0)
                        {
                            var edge = m_Graph.GetEdge(cpnode.outputPorts[i].edges[0]);
                            res.True = Convert(m_Graph.FindNode(edge.inputNodeId));
                        }
                        
                    }
                    else if (cpnode.outputPorts[i].portName == "False")
                    {
                        if (cpnode.outputPorts[i].edges != null && cpnode.outputPorts[i].edges.Count > 0)
                        {
                            var edge = m_Graph.GetEdge(cpnode.outputPorts[i].edges[0]);
