...
namespace TaoTie
{
    ...
    {
        static Dictionary<string, System.Collections.Generic.List<string>> refDic =
        ...
        public static void FindAssetsByArtToolsWindow(string[] assetsPath)
        {
            ...
            foreach (var item in guids)
            {
                ...
                if (!checkAssetPaths.ContainsKey(assetPath))
                {
                    ...
                    {
                        string[] fileStrs = Directory.GetFiles(Path.GetFullPath(assetPath), "*.*",
                            SearchOption.AllDirectories);
                        foreach (var fileItem in fileStrs)
                        {
                            if (!string.Equals(Path.GetExtension(fileItem), ".meta",
                    ...
                    {
                        if (!string.Equals(Path.GetExtension(assetPath), ".meta",
                                System.StringComparison.OrdinalIgnoreCase))
                        {
            ...
            string[] files = Directory.GetFiles(Application.dataPath, "*.*", SearchOption.AllDirectories)
                .Where(s => withoutExtensions.Contains(Path.GetExtension(s).ToLower()) &&
                            !s.Contains("AddressableAssetsData")).ToArray();
        ...
        static private void MatchFile(string[] allassetpaths, int index, string[] files,
            Dictionary<string, System.Collections.Generic.List<string>> refDic)
        {
        ...
        //查找CS文件对资源的引用
        static private void FindCSFileReferences(string[] allassetpaths,
            Dictionary<string, System.Collections.Generic.List<string>> refDic)
        {
        ...
        static private void MatchFileContents(string[] allassetpaths, int index, string[] files,
            Dictionary<string, System.Collections.Generic.List<string>> refDic, int output, bool isSpecial = false)
        {
            if (index > 0 && !string.IsNullOrEmpty(allassetpaths[index - 1]))
            {
                string guid = AssetDatabase.AssetPathToGUID(GetRelativeAssetsPath(allassetpaths[index - 1]));
            ...
            if (showNoUser)
            {
                ...
                foreach (var item in refDic)
                {
                    if (item.Value.Count == 0)
                    {
                        ...
                        GUILayout.Label(
                            "内存占用：" + EditorUtility.FormatBytes(Profiler.GetRuntimeMemorySizeLong(assetObj)),
                            GUILayout.Width(150));
        ...
        static private void OutputToExcel()
        {
            ...
            string timeStr = time.Year.ToString("D4") + "_" + time.Month.ToString("D2") + time.Day.ToString("D2") +
            ...
            using (ExcelPackage package = new ExcelPackage(newFile))
            {
                ...
                foreach (var item in refDic)
                {
                    ...
                    worksheet.Cells["C" + index].Value =
                        EditorUtility.FormatBytes(Profiler.GetRuntimeMemorySizeLong(assetObj));
