...
namespace TaoTie
{
    ...
    public class CheckUnuseImage : EditorWindow
    {
        ...
        private void StartCheck()
        {
            ...
            imagePaths = imagePaths.Where(s => !Regex.IsMatch(s, "ColorPokerCard"))
                .Where(s => !Regex.IsMatch(s, "PokerCard")).ToList();


            List<string> withoutExtensions = new List<string>() {".prefab", ".unity", ".mat", ".cs", ".anim"};
            string[] files = Directory.GetFiles(Path.Combine(Application.dataPath, "AssetsPackage"), "*.*",
            ...
            files = Directory.GetFiles("Assets/Scripts/Code", "*.*", SearchOption.AllDirectories)
                .Where(s => withoutExtensions.Contains(Path.GetExtension(s).ToLower())).ToArray();
