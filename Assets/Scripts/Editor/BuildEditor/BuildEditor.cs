...
namespace TaoTie
{
	...
	public class BuildEditor : EditorWindow
	{
		...
		private WebGLPlatform webGLPlatform;
        ...
        private void OnGUI() 
		{
			...
			{
				channel = webGLPlatform.ToString();
			...
			if (platformType == PlatformType.WebGL)
			{
				...
				if (webGLPlatform != WebGLPlatform.WebGL 
				    && webGLPlatform != WebGLPlatform.FaceBook
				    && webGLPlatform != WebGLPlatform._4399)
				{
...
#elif UNITY_WEBGL_KS
				        var win = GetWindow(typeof(KSWASM.editor.MiniHostBuildWindow), false, "构建快手小游戏");
				        win.minSize = new Vector2(750, 400);
				        win.position = new Rect(100, 100, 750, 400);
				        win.Show();
#elif UNITY_WEBGL_TAPTAP
				        var win = GetWindow(typeof(TapTapMiniGame.TapTapBuildWindow), false, "构建TapTap小游戏");
				        win.minSize = new Vector2(750, 400);
				        win.position = new Rect(100, 100, 750, 400);
				        win.Show();
#elif UNITY_WEBGL_BILIGAME
						WeChatWASM.WXEditorWindow.Open();
#elif UNITY_WEBGL_ALIPAY
						AlipayEditorWindow.OpenAlipayEditor();
#elif UNITY_WEBGL_QG
						QGMiniGameCore.QGEditorWindowNew.OpenWindow();
#elif UNITY_WEBGL_MINIHOST
						minihost.editor.MiniHostBuildWindow.ShowWindow();
#endif
					}
				}
			}
			...
			if (GUILayout.Button("开始打包"))
			{
				...
				if (collectShaderVariant)
				{
					BuildHelper.CollectSVC((res) =>
					{
						...
						{
							BuildHelper.Build(platformType, buildOptions, isBuildExe, clearReleaseFolder, clearABFolder,
								buildHotfixAssembliesAOT, isBuildAll, isPackAtlas, isBuildAll || isContainsAb, channel,
