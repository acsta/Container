...
namespace TaoTie
{
    ...
    public abstract class UIBaseContainer
    {
        ...
        public async ETTask Shake(int force = 1, int during = 1000, float hz = 50)
        {
            ...
            force *= 10;
            ActivatingComponent();
            var startPos = GetRectTransform().anchoredPosition;
            isShake = true;
            hz = Mathf.PI * hz / 500;
            while (transform != null)
            {
                ...
                GetRectTransform().anchoredPosition = startPos + force * Vector2.up * Mathf.Sin(deltaTime * hz) / addon;

                if (deltaTime >= during)
                {
                    GetRectTransform().anchoredPosition = startPos;
                    break;
                }
            }
        ...
        public async ETTask ScaleTo(Vector3 scale, int during = 500, EasingFunction.Ease easing = EasingFunction.Ease.Linear)
        {
            ...
            var startTime = TimerManager.Instance.GetTimeNow();
            ActivatingComponent();
            var startScale = GetRectTransform().localScale;
            isScale = true;
            ...
            while (transform != null)
            {
                ...
                var value = easingFunction.Invoke(deltaTime, 0, 1, during);
                GetRectTransform().localScale = Vector3.Lerp(startScale, scale, value);

                if (deltaTime >= during)
                {
                    GetRectTransform().localScale = Vector3.Lerp(startScale, scale, easingFunction.Invoke(1, 0, 1));
                    break;
                }
            }
