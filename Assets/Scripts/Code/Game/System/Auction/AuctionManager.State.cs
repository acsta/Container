...
namespace TaoTie
{
    public partial class AuctionManager:IUpdate
    {
        ...
        private void Awake()
        {
	        ...
	        //选择情报
	        CreateGameInfo();
	        if (CacheManager.Instance.GetInt(CacheKeys.DiceSetting) == 1 && DiceConfigCategory.Instance.TryGetDiceConfig(Level, out _))
	        {
		        UIManager.Instance.OpenWindow<UIDiceWin>(UIDiceWin.PrefabPath,UILayerNames.TipLayer).Coroutine();
	        }
            ...
            bool hasBlack = Random.Range(0, 100) < LevelConfig.BlackPercent;
            int blackCount = 0;
            if (hasBlack)
            {
	            blackCount = Random.Range(1, LevelConfig.BlackCount + 1);
            }
            for (i = 0; i < len; i++)
            {
	            var bidder =
		            EntityManager.Instance.CreateEntity<Bidder, int, bool>(LevelConfig.AIIds[i], i < blackCount);
                Bidders.Add(bidder.Id);
                var index = 0;
                if (i < blackCount)
                {
	                Blacks.Add(bidder.Id);
                }
                else
                {
	                index = Random.Range(0, target.Count - reduce);
                }
                root = target[index];
        ...
        private void Prepare()
        {
	        if (!isEnterState)
	        {
		        if (GameInfoId != 0 && UIManager.Instance.GetView<UIDiceWin>(1) == null)
		        {
	        ...
	        //生成集装箱
	        CreateContainer();
	        PlayerDataManager.Instance.AddPlayCount(Level);
        }
        /// <summary>
        /// 再来一局
        /// </summary>
        ...
        private void Ready()
        {
	        ...
	        Skip = false;
	        //从抬价开始
	        if (DiceConfig.Type == 5)
	        {
		        Log.Info("从抬价阶段开始");
		        IsRaising = true;
		        LastAuctionPrice = AllPrice;
		        SysJudgePriceMax = 1;
	        }
        ...
        private void AllOver()
        {
            ...
            UIManager.Instance.OpenWindow<UIReportWin, AuctionReport[], int>
	            (UIReportWin.PrefabPath, AuctionReports, Level).Coroutine();
            ...
            if (hasGodOfWealth)
            {
	            ...
	            {
		            if (AuctionReports[i].Type == ReportType.Self)
		            {
			            total += AuctionReports[i].AllPrice;
		            }
	            }
	            UIManager.Instance.OpenWindow<UITurnTableEventView, BigNumber, int>
		            (UITurnTableEventView.PrefabPath, total, Level).Coroutine();
            }
        }
